    ...
    switch (newStatus) {
      ...
      case "accepted": {
        if (hasTag(currentTags, config.suggestionTags.accepted)) {
          return ctx.editReply(
            "This suggestion is already marked as accepted."
          );
        }
      ...
      case "rejected": {
        if (hasTag(currentTags, config.suggestionTags.rejected)) {
          return ctx.editReply(
            "This suggestion is already marked as rejected."
          );
        }
      ...
      case "duplicate": {
        if (hasTag(currentTags, config.suggestionTags.duplicate)) {
          return ctx.editReply(
            "This suggestion is already marked as duplicate."
          );
        }
        ...
        await ctx.editReply(
          "Suggestion marked as duplicate and thread locked."
        );
        ...
        return ctx.channel.edit({
          ...
          appliedTags: addTag(
            tagsWithoutStatus,
            config.suggestionTags.duplicate
          ),
        });
      }
